...
CMN_HOST_VALUE="${CMN_HOST:-}"
CMN_SFTP_HOST_VALUE="${CMN_SFTP_HOST:-}"
CMN_SFTP_USER_VALUE="${CMN_SFTP_USER:-}"
CMN_SFTP_PORT_VALUE="${CMN_SFTP_PORT:-22}"
CMN_VERSION_VALUE="${CMN_VERSION:-}"
CMN_TOKEN_VALUE="${CMN_TOKEN:-}"
CMN_REMOTE_PATH_VALUE="${CMN_REMOTE_PLUGIN_DIR:-$DEFAULT_REMOTE_PLUGIN_DIR}"
...
Options:
  --host <host>            Website host for release endpoint (default: CMN_HOST)
  --sftp-host <host>       SSH/SFTP host (default: CMN_SFTP_HOST or --host)
  --user <user>            SSH/SFTP user (default: CMN_SFTP_USER)
  --port <port>            SSH/SFTP port (default: CMN_SFTP_PORT or 22)
  --remote-path <path>     Remote plugin path (default: $DEFAULT_REMOTE_PLUGIN_DIR)
  ...
  --dry-run                Print plan + local/remote versions/hashes, no upload
  --verify-only            Skip upload, run post-deploy verification only
  --help                   Show this help

Examples:
  ./deploy_portal.sh --sftp-host access-5018438942.webspace-host.com --user su19353 --dry-run
  ./deploy_portal.sh --sftp-host access-5018438942.webspace-host.com --user su19353 --version 0.1.25 --token "***"
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --host)
      ...
      ;;
    --sftp-host)
      CMN_SFTP_HOST_VALUE="${2:-}"
      shift 2
      ;;
    --user)
      CMN_SFTP_USER_VALUE="${2:-}"
      shift 2
      ;;
    --port)
      CMN_SFTP_PORT_VALUE="${2:-}"
      shift 2
      ;;
    --remote-path)
      CMN_REMOTE_PATH_VALUE="${2:-}"
      shift 2
      ;;
    --version)
      CMN_VERSION_VALUE="${2:-}"
...
if [[ -z "$CMN_SFTP_HOST_VALUE" && -n "$CMN_HOST_VALUE" ]]; then
  CMN_SFTP_HOST_VALUE="$CMN_HOST_VALUE"
fi
if [[ -z "$CMN_HOST_VALUE" && -n "$CMN_SFTP_HOST_VALUE" ]]; then
  CMN_HOST_VALUE="$CMN_SFTP_HOST_VALUE"
fi

if [[ -z "$CMN_SFTP_HOST_VALUE" ]]; then
  echo "Missing required host. Provide --sftp-host (or CMN_SFTP_HOST)." >&2
  exit 1
fi
if [[ -z "$CMN_SFTP_USER_VALUE" ]]; then
  echo "Missing required user. Provide --user (or CMN_SFTP_USER)." >&2
  exit 1
fi
if [[ -z "$CMN_REMOTE_PATH_VALUE" ]]; then
  echo "Missing remote plugin path." >&2
  exit 1
fi

if ! [[ "$CMN_SFTP_PORT_VALUE" =~ ^[0-9]+$ ]]; then
  echo "Invalid port: $CMN_SFTP_PORT_VALUE" >&2
  exit 1
fi
...
SSH_TARGET="$CMN_SFTP_USER_VALUE@$CMN_SFTP_HOST_VALUE"
...
print_plan() {
  ...
  echo "Remote host/user:   $SSH_TARGET:$CMN_SFTP_PORT_VALUE"
  echo "Remote plugin path: $CMN_REMOTE_PATH_VALUE"
...
mapfile -t before_probe < <(remote_probe "$CMN_REMOTE_PATH_VALUE")
...
-mkdir $CMN_REMOTE_PATH_VALUE
lcd $SCRIPT_DIR
put covermenowone-one.php $CMN_REMOTE_PATH_VALUE/covermenowone-one.php
put frontend.css $CMN_REMOTE_PATH_VALUE/frontend.css
put frontend.js $CMN_REMOTE_PATH_VALUE/frontend.js
put livechat-widget.css $CMN_REMOTE_PATH_VALUE/livechat-widget.css
put livechat-widget.js $CMN_REMOTE_PATH_VALUE/livechat-widget.js
put admin.css $CMN_REMOTE_PATH_VALUE/admin.css
put login.css $CMN_REMOTE_PATH_VALUE/login.css
put -r assets $CMN_REMOTE_PATH_VALUE
put -r cv-converter $CMN_REMOTE_PATH_VALUE
SFTP_CMDS
...
mapfile -t after_probe < <(remote_probe "$CMN_REMOTE_PATH_VALUE")
...
if [[ "$remote_before_sha" == "$remote_after_sha" ]]; then
  echo "Note: frontend.js hash unchanged compared with pre-deploy snapshot."
else
  echo "frontend.js hash updated on remote."
fi
